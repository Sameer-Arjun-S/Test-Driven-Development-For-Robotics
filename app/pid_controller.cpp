 ...
 */
pid_controller::pid_controller(double Kp_input, double Ki_input,double Kd_input, double dt_input) {
  // Assigning the inputs to the variables
  Kp = Kp_input;
  Ki = Ki_input;
  Kd = Kd_input;
  dt = dt_input;
  std::cout << "The inputs from the user are " << "Kp:" << Kp << " Kd:"
 ...
 */
double pid_controller::returndt() {
  return 0;
}
